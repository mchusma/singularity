...
const ActiveUnits = () => {
  ...
  const unitWidth = screenWidth > 1200 ? screenWidth * 0.3 : screenWidth >= 800 ? screenWidth * 0.48 : screenWidth * 0.98;  const units = useSelector((state: RootState) => state.units.units);
        ...
        if (!unit.isVisible) {
          ...
          const requirementsMet = unit.requiredUnits.every(
            (requiredUnit: RequiredUnit) => {
              const requiredUnitState = unitsRef.current.find(
                (u: Unit) => u.id === requiredUnit.unitId
              );
              if (!requiredUnitState) {
                console.error(
                  `Required unit with id ${requiredUnit.unitId} not found`
                );
              }
              return (
                requiredUnitState &&
                requiredUnitState.quantity >= requiredUnit.quantity
              );
            }
          );
  ...
  const visibleUnitsWithState = activeUnits
    .map((UnitComponent) => {
      const unitState = units.find(
        (unit) => unit.id === UnitComponent.name.toLowerCase()
      );
      if (!unitState) {
        console.error(
          `Unit with id ${UnitComponent.name.toLowerCase()} not found in state`
        );
      }
  ...
  const sortedVisibleUnits = visibleUnitsWithState
    .sort((a, b) => (a.order || 0) - (b.order || 0))
    .map((unit) => unit.UnitComponent);
  return (
    <View>
